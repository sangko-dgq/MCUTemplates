...
void	DisableEEPROM(void)
{
	...
	IAP_ADDRE = 0x00;
	IAP_ADDRH = 0x00;		//清0地址高字节
	IAP_ADDRL = 0x00;		//清0地址低字节，指向非EEPROM区，防止误操作
}
//========================================================================
...
void EEPROM_read_n(unsigned int EE_address, unsigned char *DataAddress, unsigned int number)
{
	...
	do
	{
		IAP_ADDRL = EE_address;				//送地址低字节
		IAP_ADDRH = EE_address >> 8;	//送地址高字节（地址需要改变时才需重新送地址）
		IAP_ADDRE = 0x00;
...
void EEPROM_SectorErase(unsigned int EE_address)
{
	...
	IAP_TPS = 35;
	IAP_CMD   = CMD_ERASE;
	IAP_ADDRL = EE_address;				//送扇区地址低字节
	IAP_ADDRH = EE_address >> 8;	//送扇区地址高字节（地址需要改变时才需重新送地址）
	IAP_ADDRE = 0x00;
	IAP_TRIG  = 0x5a;	IAP_TRIG  = 0xa5;
	_nop_();
	_nop_();
